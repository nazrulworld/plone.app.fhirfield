...
FHIR_STATIC_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "browser", "static", "FHIR"
)
...
with open(
    os.path.join(
        FHIR_STATIC_DIR, "HL7", "search", "FHIR-Search-Parameter-Registry.json"
    )
) as f:
...
with open(
    os.path.join(
        ...
        "HL7",
        "search",
        "FHIR-Search-Parameter-Registry-searchable.json",
    )
) as f:
...
with open(os.path.join(FHIR_RESOURCE_LIST_DIR, FHIR_VERSION + ".json"), "r") as f:
    """ """
