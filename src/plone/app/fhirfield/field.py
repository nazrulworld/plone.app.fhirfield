...
class FhirResource(Object):
    ...
    def _init_validate(self):
        ...
        if self.model_interface:
            try:
                ...
                msg = (
                    "Invalid FHIR Model Interface`{0}`! "
                    "Please check the module or class name.".format(
            ...
            if klass is not IFhirResourceModel and not issubclass(
                klass, IFhirResourceModel
            ):
                msg = "`{0!r}` must be derived from {1}".format(
                    klass,
                    IFhirResourceModel.__module__
                    + "."
                    + IFhirResourceModel.__class__.__name__,
                )
    ...
    def _from_dict(self, dict_value):
        ...
        value = FhirResourceValue(raw=klass(dict_value), encoding="utf-8")
    ...
    def _validate(self, value):
        ...
        if self.resource_type and value.resource_type != self.resource_type:
            msg = (
                "Resource type must be `{0}` "
                "but we got {1} which is not allowed!".format(
